...
def baseline(iterations, traject_type, min_trajects, max_trajects, stations_file, connections_file):
    """
    Run a baseline trajectory generation multiple times and collect quality scores.
    """
    quality_dict = {1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[]}
    for i in range(iterations):
        nr, trajects, quality = random_multiple_trajects(traject_type, min_trajects, max_trajects, stations_file, connections_file)
        quality_dict[nr].append(quality)

    return quality_dict


def collect_baselines(iterations, traject_type, num_runs, min_trajects, max_trajects, stations_file, connections_file):
    """
    ...
    for run in range(num_runs):
        quality_dict = baseline(
            iterations, traject_type, min_trajects, max_trajects, stations_file, connections_file)
...
def plot_quality_distribution(all_results, iterations):
    ...
    sns.histplot(data=df, x="Quality", hue="Trajectories", multiple="stack", bins=30, legend=True)
